...
@Composable
fun ProfileRegistr(navController: NavController) {
    Column() {
        TopAppBar(
    ...
    Column(Modifier.padding(top = 420.dp, start = 20.dp)) {
        ...
        Text(text = "Регистрация", color = Color(0xFF6F6F6F))
        ...
        if (phone.length == 11){
            OutlinedButton(onClick = {  }, shape = RoundedCornerShape(15),
                modifier = Modifier
                    .padding(top = 20.dp, start = 0.dp)
                    .size(320.dp, 45.dp)
                    .border(2.dp, color = Color(0xFF644AA9), shape = RoundedCornerShape(15))) {
                Text(text = "Получить код", fontSize = 17.sp, color = Color(0xFF644AA9))
            }
        }
